...
class TradeMatch(Trainer):
    def __init__(self, cfg, device, all_cfg, **kwargs):
        super(TradeMatch, self).__init__(cfg, device)
    ...
    def compute_loss(self, data_x, data_u, model, ema_model=None, **kwargs):
        ...
        Lu = (mask * (self.loss_u(logits_u_w, p_targets_u, reduction='none') + self.beta * F.kl_div(F.log_softmax(logits_u_s, dim=-1), F.softmax(logits_u_w.detach(), dim=-1), reduction="none").sum(dim=-1))).mean()
        # Lu = (mask * self.loss_x(logits_u_w, p_targets_u, reduction='none')).mean() + self.beta * (max_probs * F.kl_div(F.log_softmax(logits_u_s, dim=-1), F.softmax(logits_u_w.detach(), dim=-1), reduction="none").sum(dim=-1)).mean()
